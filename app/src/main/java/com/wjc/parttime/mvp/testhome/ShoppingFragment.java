package com.wjc.parttime.mvp.testhome;
...
import com.wjc.parttime.mvp.testhome.adapter.PintuanAdapter;
import com.wjc.parttime.mvp.testhome.adapter.ShopAdapter;
...
public class ShoppingFragment extends MVCBaseFragment {
...
//            Picasso.with(getActivity()).load(images[i]).resize(621, 237).centerCrop().into(mView);
            Glide.with(getActivity()).load(images[i]).override(621,237).centerCrop().into(mView);
            mViewList.add(mView);
        }
